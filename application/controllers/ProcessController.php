...
//        ambil model datamodel
    ...
    public function getData() {
        ...
        $hasilPerhitungan = $this->perhitungan($data);
        $response = array_merge($dataAwal, $hasilPerhitungan);
    ...
    public function printData() {
        $po = $this->input->get("po");
        $cek = $this->DataModel->cekPo($po);
        echo count($cek);
    }

    public function printWord() {
        $po = urldecode($this->uri->segment(2));
        $dataMurni = $this->DataModel->ambilDataMurni($po);
        $dataAwal = array();
        for ($i = 0; $i < count($dataMurni); $i++) {
            array_push($dataAwal, $dataMurni[$i]['rupiah']);
        }
        $peramalan = $this->perhitungan->perhitunganAlpha($dataAwal, 0.1);
        $peramalanDesember2017 = ((0.1 * $dataAwal['48'] + 0.9 * $peramalan[58]));
        array_push($peramalan, $peramalanDesember2017);
        $hasilPerhitungan = $this->perhitungan($dataAwal);
        $data = array(
            "awal" => $dataAwal,
            "peramalan" => $peramalan,
            "po"=>$po
        );
        $response = array_merge($data, $hasilPerhitungan);

        $this->load->view("eko/print", $response);
    }
